...
class Tournament:
    ...
    def save(self):
        ...
        result_tournament = self.db_tournaments.search(
            self.tournaments_query.id == self.id
        )
        if not result_tournament:
            ...
            self.db_tournaments.update(
                self.to_dict(), self.tournaments_query.id == self.id
            )
    ...
    def search(key, query):
        result = Tournament.db_tournaments.search(
            (Tournament.tournaments_query[key] == query)
        )
        if result:
            return result[0]
        return False
    ...
    def list(filter):
        ...
        if filter == "all":
            ...
            return Tournament.db_tournaments.search(
                Tournament.tournaments_query.end_date == ""
            )
